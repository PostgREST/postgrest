...
import System.Process.Restart         (RestartOptions (..),
                                       SignalAction (..), defaultRestartOptions,
                                       handleSignal, isStandalone,
                                       runRestartable, sigHUP, sigINT, sigTERM,
                                       sigUSR1, sigUSR2)
    ...
    restartOptions AppConfig{..} =
      defaultRestartOptions
        { restartSignalHandlers =
            mconcat
              [ handleSignal sigINT $ observer (TerminationUnixSignalObs "SIGINT") $> Stop
              , handleSignal sigTERM $ observer (TerminationUnixSignalObs "SIGTERM") $> Stop
              , handleSignal sigUSR1 $ AppState.schemaCacheLoader appState $> None
              , handleSignal sigUSR2 $ AppState.readInDbConfig False appState $> None
              , if configServerReusePort && isNothing configServerUnixSocket && isNothing configAdminServerUnixSocket then
                  handleSignal sigHUP $ pure Restart
                else
                  mempty
              ]
        }
