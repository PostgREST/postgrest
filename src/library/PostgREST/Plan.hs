...
inspectPlan apiRequest headersOnly schema = do
  ...
  mediaType <-
    if not . null $ L.intersect accepts producedMTs
      ...
      let
        newReadPlan =
          ( \r ->
            ...
            <$> rel
        origin =
          if depth == 1 -- Only on depth 1 we check if the root(depth 0) has an alias so the sourceCTEName alias can be found as a relationship
...
applySpreadAggHoistingToNode (Node rp@ReadPlan{relAggAlias, relToParent, relSpread} children) =
  let
    ...
    (newSelects, aggList) =
      if depth rp == 0 || (isJust relToParent && not isToOneSpread)
        then (select rp, [])
    ...
    newRelSelects =
      if null children || isToOneSpread
        then relSelect rp
