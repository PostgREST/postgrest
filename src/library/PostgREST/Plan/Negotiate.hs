...
negotiateContent conf ApiRequest{iAction = act, iPreferences = Preferences{preferRepresentation = rep}} identifier accepts produces defaultSelect =
  ...
  where
    ...
    lookupHandler mt =
      when' defaultSelect (HM.lookup (RelId identifier, MTAny) produces)
        <|> when' defaultSelect (HM.lookup (RelId identifier, mt) produces) -- lookup for identifier and `*/*`
        <|> HM.lookup (RelAnyElement, mt) produces -- lookup for identifier and a particular media type
        -- lookup for anyelement and a particular media type
