...
init conf@AppConfig{configLogLevel, configDbPoolSize} = do
  ...
  state' <- initWithPool (sock, adminSock) pool conf loggerState metricsState observer jwtCacheState
  pure state' { stateSocketREST = sock, stateSocketAdmin = adminSock}

initWithPool :: AppSockets -> SQL.Pool -> AppConfig -> Logger.LoggerState -> Metrics.MetricsState -> ObservationHandler -> JwtCacheState -> IO AppState
initWithPool (sock, adminSock) pool conf loggerState metricsState observer jwtCacheState = do
