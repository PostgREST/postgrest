...
main CLI{cliCommand, cliPath} = do
  ...
  case cliCommand of
    ...
    Run runCmd      -> do
      let runApp tracerSt = runAppCommand tracerSt conf runCmd
      if configOTelEnabled conf
        then withTracer $ \tracer -> runApp (TracerState $ Just tracer)
        else runApp emptyTracerState
