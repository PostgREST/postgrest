...
import PostgREST.OpenTelemetry (tracerIsEnabled, withTracer)
...
main CLI{cliCommand, cliPath} = withTracer $ \tracer -> do
  ...
  let tracerSt = TracerState {ts'enabled = configOTelEnabled conf, ts'otelTracer = if tracerIsEnabled tracer then Just tracer else Nothing}

  case cliCommand of
    Client adminCmd -> runClientCommand conf adminCmd
    Run runCmd      -> runAppCommand tracerSt conf runCmd
...
runAppCommand :: TracerState -> AppConfig -> RunCommand -> IO ()
runAppCommand tracerState conf@AppConfig{..} runCmd = do
  -- Per https://github.com/PostgREST/postgrest/issues/268, we want to
  -- explicitly close the connections to PostgreSQL on shutdown.
  -- 'AppState.destroy' takes care of that.
  bracket
    (AppState.init conf tracerState)
    AppState.destroy
