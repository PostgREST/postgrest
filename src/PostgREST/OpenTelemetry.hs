...
In order for produced spans to have correct code locations, all the functions across the call stack up to the
...
-}
module PostgREST.OpenTelemetry (Tracer, withTracer, inSpanM, inSpanMDefault, oTelMiddleware, oTelMiddlewareDynamic) where
...
oTelMiddleware :: Maybe Tracer -> Middleware
oTelMiddleware (Just t) = newOpenTelemetryWaiMiddleware' $ getTracerTracerProvider t
oTelMiddleware Nothing  = identity

-- | Dynamic OpenTelemetry middleware that retrieves the tracer using an IO action
oTelMiddlewareDynamic :: IO (Maybe Tracer) -> Middleware
oTelMiddlewareDynamic getTracerAction app req respond = do
  currentTracer <- getTracerAction
  let staticMiddleware = oTelMiddleware currentTracer
  staticMiddleware app req respond
