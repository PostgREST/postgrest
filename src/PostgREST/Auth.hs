...
parseToken AppConfig{..} (Just tkn) time = do
  -- validate if the token has 3 period separated parts and min of 32 chars
  min32CharToken <- liftEither $ hasMin32Chars tkn
  tknWithThreeParts <- liftEither $ hasThreeParts min32CharToken
  secret <- liftEither . maybeToRight (JwtErr JwtSecretMissing) $ configJWKS
  eitherContent <- liftIO $ JWT.decode (JWT.keys secret) Nothing tknWithThreeParts
