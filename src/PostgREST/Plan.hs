...
mutatePlan mutation qi ApiRequest{iPreferences=Preferences{..}, ..} SchemaCache{dbTables, dbRepresentations} readReq =
  case mutation of
    MutationCreate ->
      mapRight (\typedColumns -> Insert qi typedColumns body ((,) <$> preferResolution <*> Just confCols) [] returnings pkCols applyDefaults) (typedColumnsOrError iColumnsList)
    MutationUpdate ->
      mapRight (\typedColumns -> Update qi typedColumns body combinedLogic returnings applyDefaults) (typedColumnsOrError iColumnsList)
    MutationSingleUpsert ->
        if null qsLogic &&
          ...
          then mapRight (\typedColumns -> Insert qi typedColumns body (Just (MergeDuplicates, pkCols)) combinedLogic returnings mempty False) (typedColumnsOrError iColumnsList)
        else
    ...
    MutationPgrstPatch ->
      case pgrstPatchBody of
        Just bdy ->
          -- If we have columns query param, then update only those columns.
          -- Otherwise, update all the fields given in Pgrst Patch body.
          mapRight (\typedColumns -> PgrstPatch qi typedColumns pgrstPatchBody combinedLogic returnings) (typedColumnsOrError $ if null iColumnsList then map getPgrstPatchFieldName bdy else iColumnsList)
        Nothing ->
          Left $ ApiRequestError $ InvalidBody "PGRST Patch Body is missing"
  where
    iColumnsList = S.toList iColumns
