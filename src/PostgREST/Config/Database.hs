...
queryRoleSettings pgVer prepared =
  ...
  where
    ...
    -- Quote it to allow upper case or mixed case role names
    currentUser = "quote_ident(current_user)::regrole::oid"

    hasParameterPrivilege
      | pgVer >= pgVersion150 = "or has_parameter_privilege(" <> currentUser <> ", ps.name, 'set')"
      | otherwise             = ""
