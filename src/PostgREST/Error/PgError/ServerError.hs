...
module PostgREST.Error.PgError.ServerError
  ( pgErrorStatus,
    maybeHeaders,
  ) where
...
import qualified PostgREST.Error.PgError.ServerError.RaisePgrst as RaisePgrst

import PostgREST.Error.Algebra
import Protolude

instance ErrorBody SQL.ServerError where
  ...
  code (SQL.ServerError "PGRST" m d _ _) =
    case RaisePgrst.parseRaisePGRST m d of
      ...
      Left e       -> code e
  code (SQL.ServerError c _ _ _ _) = c

  message (SQL.ServerError "PGRST" m d _ _) =
    case RaisePgrst.parseRaisePGRST m d of
      Right (r, _) -> RaisePgrst.getMessage r
      Left e       -> message e
  message (SQL.ServerError _ m _ _ _) = m

  details (SQL.ServerError "PGRST" m d _ _) =
    case RaisePgrst.parseRaisePGRST m d of
      Right (r, _) -> JSON.String <$> RaisePgrst.getDetails r
      Left e       -> details e
  details (SQL.ServerError _ _ d _ _) = JSON.String <$> d

  hint (SQL.ServerError "PGRST" m d _ _p) =
    case RaisePgrst.parseRaisePGRST m d of
      Right (r, _) -> JSON.String <$> RaisePgrst.getHint r
      Left e       -> hint e
  hint (SQL.ServerError _ _ _ h _) = JSON.String <$> h

pgErrorStatus :: Bool -> SQL.ServerError -> HTTP.Status
pgErrorStatus authed (SQL.ServerError c m d _ _) = case T.unpack c of
...
maybeHeaders :: SQL.ServerError -> Maybe [HTTP.Header]
