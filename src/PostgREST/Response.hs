...
-- PATCH Response
actionResponse (DbCrudResult MutateReadPlan{mrMutation=MutationUpdate _, mrMedia} resultSet) ctxApiRequest@ApiRequest{iPreferences=Preferences{..}} _ _ _ _ _ = case resultSet of
  RSStandard{..} -> do
    let
      contentRangeHeader =
