...
postgrest logLevel appState connWorker =
  ...
  Logger.middleware logLevel Auth.getRole $
    ...
    \req respond -> {-# SCC "wai_request_span" #-} inSpanM (getOTelTracer appState) "request" defaultSpanArguments $
      case fromJust $ Auth.getResult req of
        ...
        Right authResult -> do
          ...
          let
            ...
            eitherResponse =
              {-# SCC "postgrest_response" #-} runExceptT $ postgrestResponse appState appConf maybeSchemaCache authResult req
...
postgrestResponse appState conf@AppConfig{..} maybeSchemaCache authResult@AuthResult{..} req = do
  ...
  (parseTime, apiReq@ApiRequest{..}) <-  {-# SCC "otel_parse" #-} withOTel "parse" $ withTiming $ liftEither . mapLeft Error.ApiRequestError $ ApiRequest.userApiRequest conf prefs req body
  (planTime, plan)                   <- {-# SCC "otel_plan" #-} withOTel "plan" $ withTiming $ liftEither $ Plan.actionPlan iAction conf apiReq sCache
  ...
  (txTime, txResult) <- {-# SCC "otel_query" #-} withOTel "query" $ withTiming $ do
    case tx of
  ...
  (respTime, resp) <- {-# SCC "otel_response" #-} withOTel "response" $ withTiming $ do
  ...
  where
    ...
    withOTel label = {-# SCC "withOTel_wrapper" #-} do
