...
clientErrorMsg NoAdminServer                 = "ERROR: Admin server is not running. Please check admin-server-port config."
clientErrorMsg (NoSpecialHostNamesAllowed host)
  = "ERROR: The `--ready` flag cannot be used when server-host is configured as \"" <> host <> "\". "
  <> "Please update your server-host config to \"localhost\"."
clientErrorMsg (PostgRESTNotReady url)       = "ERROR: " <> url
clientErrorMsg (HTTPConnectionRefused url)   = "ERROR: connection refused to " <> url
clientErrorMsg (HTTPExceptionInvalidURL url) = "ERROR: invalid url - " <> url
