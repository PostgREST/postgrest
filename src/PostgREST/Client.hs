...
clientErrorMsg NoAdminServer                 = "UNREACHABLE: Admin server is not running. Please check admin-server-port config."
clientErrorMsg (NoSpecialHostNamesAllowed host)
  = "REFUSED: The --ready flag cannot be used when server-host is configured as \"" <> host <> "\". Please update your server-host config to \"localhost\"."
