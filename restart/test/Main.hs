...
main = do
  ...
  port <- getPort
  Restart.runRestartable restartOptions (throwTo mainThreadId ThreadKilled) $ \mode ready -> do
...
restartOptions :: Restart.RestartOptions
restartOptions =
  Restart.defaultRestartOptions
    { Restart.restartSignalHandlers =
        Restart.handleSignal Restart.sigHUP $ pure Restart.Restart
    }
