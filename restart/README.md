...
This directory contains the source code for the `postgrest:restart` internal
library.
...
The `library restart` stanza in `postgrest.cabal` always includes `src/common`
and adds either `src/posix` or `src/windows` through platform-specific
`hs-source-dirs`.
