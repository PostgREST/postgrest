...
runRestartable :: RestartOptions -> AppRun a -> IO a
runRestartable RestartOptions{restartProcessConfig, restartSignalHandlers} runApp = do
  processCfg <- maybe currentProcessConfig pure restartProcessConfig
  Impl.runRestartable processCfg $ \mode ready ->
