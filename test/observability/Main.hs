...
main = do
  ...
  let
    ...
    initJwtApp = initApp baseSchemaCache testCfgJwtCache emptyTracerState

    initMetricsApp = initApp baseSchemaCache testCfg emptyTracerState
    ...
    initOTelApp action = do
      ...
      case mCollectorBin of
        Nothing -> do
          ...
          (_, app) <- initApp baseSchemaCache testCfgOTel AppState.emptyTracerState
          action (Nothing, app)
        Just collectorBin -> do
          ...
          bracket
            ...
            (\collector -> do
              ...
              withTracer $ \tracer -> do
                (_, app) <- initApp baseSchemaCache testCfgOTel (TracerState {ts'enabled = True, ts'otelTracer = Just tracer})
  ...
  hspec $ do
    ...
    before (initApp baseSchemaCache testCfg emptyTracerState) $
