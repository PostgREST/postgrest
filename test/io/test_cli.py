...
def test_cli_ready_flag_fail_when_schema_cache_not_loaded(defaultenv, metapostgrest):
    ...
    with run(env=env, port=port) as postgrest:
        ...
        assert f"UNAVAILABLE: http://{admin_host}:{admin_port}/ready" in output
...
def test_cli_ready_flag_fail_with_http_exception(defaultenv):
    ...
    with run(env=defaultenv, port=port) as postgrest:
        ...
        assert (
            f"HTTP EXCEPTION: connection refused to http://{admin_host}:{admin_port}/ready"
            in output
        )
    ...
    with run(env=defaultenv, port=port) as postgrest:
        ...
        assert (
            f"HTTP EXCEPTION: invalid url - http://{admin_host}:{admin_port}/ready"
            in output
        )
...
def test_cli_ready_flag_fail_with_special_hostname(defaultenv):
    ...
    with run(env=defaultenv, host=host, port=port) as postgrest:
        ...
        assert (
            f'REFUSED: The --ready flag cannot be used when server-host is configured as "{host}". Please update your server-host config to "localhost".'
            in output
        )
...
def test_cli_ready_flag_fail_when_no_admin_server(defaultenv):
    ...
    with run(env=defaultenv) as postgrest:
        ...
        assert (
            "UNREACHABLE: Admin server is not running. Please check admin-server-port config."
            in output
        )
