...
def test_prefer_timeout_header_with_lenient_handling(defaultenv):
    ...
    with run(env=defaultenv) as postgrest:
        ...
        response = postgrest.session.get("/rpc/sleep?seconds=1.5", headers=headers)
...
def test_prefer_timeout_header_with_global_timeout(defaultenv):
    ...
    with run(env=env) as postgrest:
        ...
        os.system(
            f'psql -d {env["PGDATABASE"]} -U postgres -h {env["PGHOST"]} --set ON_ERROR_STOP=1 -a -c "ALTER DATABASE postgres SET statement_timeout TO \'3s\';"'
        )
        ...
        os.system(
            f'psql -d {env["PGDATABASE"]} -U postgres -h {env["PGHOST"]} --set ON_ERROR_STOP=1 -a -c "ALTER DATABASE postgres RESET statement_timeout;"'
        )
