...
@pytest.mark.parametrize("level", ["crit", "error", "warn", "info", "debug"])
def test_use_legacy_target_names(enabled, level, defaultenv):
    ...
    with run(env=env) as postgrest:
        ...
        log_err_hint = "Please update filters, orders or limits that use `films` to `all_films`, `awards` to `awards_2026` in `GET /directors?select=name,all_films:films(title),awards_2026:awards(name)&films.order=title&awards.year=eq.2026`"

        has_warning_logs = any(
            log_err_warning in line or log_err_hint in line for line in output
        )

        if enabled == "true":
            assert response.status_code == 200
            if level in ["warn", "info", "debug"]:
                assert has_warning_logs
            else:
                assert not has_warning_logs
        else:
            assert response.status_code == 400
            assert not has_warning_logs
