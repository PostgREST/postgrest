...
spec = describe "Tests using Toxiproxy" $ do
  it "Should return 503 on temporary database server unavailability" $ do
    pendingWith "Waiting for fix: https://github.com/PostgREST/postgrest/issues/4681"
    ...
    liftBaseDiscard (withDisabled specToxiProxy) $ do
      get "/items?id=eq.5" `shouldRespondWith` 503

    get "/items?id=eq.5" `shouldRespondWith` 200
