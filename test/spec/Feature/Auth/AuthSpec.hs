...
spec = describe "authorization" $ do
  ...
  it "fails when JWT length is less than 32 characters" $ do
    let auth = authHeaderJWT "tokenWithLessThan32Chars"
    request methodGet "/authors_only" [auth] ""
    `shouldRespondWith` [json| {"message":"JWT must be at least 32 characters","code":"PGRST301","hint":null,"details":null} |]
      { matchStatus = 401
      , matchHeaders = [
          "WWW-Authenticate" <:>
          "Bearer error=\"invalid_token\", error_description=\"JWT must be at least 32 characters\""
        ]
      }
