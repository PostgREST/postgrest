    ...
    context "Update Single Row" $ do
      ...
      it "should err on invalid operator" $
        request methodPatch "/test_pgrst_patch?id=eq.2"
          ...
          `shouldRespondWith`
          [json| {"code":"PGRST102","details":null,"hint":null,"message":"Error in $[0]: Unknown Pgrst Patch operation \"invalid_op\""} |]
          { matchStatus = 400
      ...
      it "should err when value other type instead of number" $
        request methodPatch "/test_pgrst_patch?id=eq.3"
          [jsonPatchCT]
          [json| [
            { "op": "set", "path": "id", "value": 20 }
          ] |]
          `shouldRespondWith`
          [json| {"code":"PGRST102","details":null,"hint":null,"message":"Error in $[0]: Expected JSON string for \"value\""} |]
          { matchStatus = 400
          , matchHeaders = [matchContentTypeJson]
          }
      ...
      it "&columns query param is not supported with Pgrst Patch" $ -- design decision pending
        request methodPatch "/test_pgrst_patch?id=eq.3&columns=name"
          [jsonPatchCT, ("Prefer","return=representation")]
          [json| [
            { "op": "set", "path": "id", "value": "10" },
            { "op": "set", "path": "name", "value": "double d" }
