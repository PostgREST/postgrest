...
allowed =
  describe "aggregate functions" $ do
    ...
    context "performing aggregations on hoisted relationships" $ do
      context "to-one hoisted relationships" $ do
        it "supports hoisting aggregates on spreaded fields" $ do
    ...
    context "performing aggregations on spreaded fields from an embedded resource" $ do
      context "to-one spread relationships" $
        -- TODO(draft): maybe remove or rename. Just an informative example that it no longer hoists the aggregate.
        it "supports the use of aggregates but does not hoist them to the top" $
          get "/process_supervisor?select=...processes(factory_id,...process_costs(cost.sum()))" `shouldRespondWith`
            [json|[{"factory_id": 1, "sum": 150.00},
                   {"factory_id": 1, "sum": 200.00},
                   {"factory_id": 2, "sum": 180.00},
                   {"factory_id": 2, "sum": 180.00},
                   {"factory_id": 2, "sum": 70.00},
                   {"factory_id": 2, "sum": 70.00},
                   {"factory_id": 3, "sum": 40.00},
                   {"factory_id": 3, "sum": 70.00}]|]
            { matchHeaders = [matchContentTypeJson] }
