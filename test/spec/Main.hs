...
main = withTracer $ \tracer -> do
  ...
  let
    initApp sCache st config = do
      let tracer' = if configServerOtelEnabled config
                        then Just tracer
                        else Nothing
      appState <- AppState.initWithPool sockets pool config loggerState metricsState tracer' (Metrics.observationMetrics metricsState)
